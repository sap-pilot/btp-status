...
4. **Starred response files** — click the ⭐ button at the end of any history table row to star the record; starring renames the response JSON and all sidecars (screenshot, console log, page source, retry files) to include `.starred.` in the filename and updates all internal cross-references atomically; starred files are retained by housekeeping indefinitely regardless of `MAX_RESPONSE_STORAGE_DAYS`, survive remote sync, and are included in availability calculations alongside regular files; unstarring reverses all renames; a **Starred** option in the tag filter shows only starred rows (`?tag=starred`)
