...
- **"Last Checked" stat card** — a fifth stat card showing the time of the last data refresh (`hh:mm:ss a` format, e.g. `02:34:56 PM`) on both the Overview and Service detail pages; it updates on every full page load and on every live delta refresh via SSE; on mobile it spans the full width of the 2-column stat grid; replaces the "Refreshed {time}" label that was in the Overview header; when the user is authenticated (or auth is disabled), clicking the card triggers an immediate sync (same effect as the Sync button)
