...
- **`SYNC_INTERVAL` default lowered to 120 s (2 minutes)** — remote sync now runs every 2 minutes by default instead of 5; the per-endpoint `interval × 2` lookback window in `GET /health/:name` means the sync gap is the practical freshness bound for Traffic Manager probes
