...
### Changed
- **`GET /health/:name` no longer runs a live probe** — the endpoint now reads the **latest saved response file** per region-matching endpoint and returns immediately; designed for Azure Traffic Manager probes running every 3–5 seconds from multiple PoPs that require sub-second responses; region is extracted from the request hostname (`cfapps.<region>.hana`) and matched against `endpoints[].region` — an endpoint with no `region` is always included; evaluation mode (`alwaysok` / `alwayserror`) still takes precedence and bypasses the file lookup; possible responses: `200 OK` (all passed), `200 Partially OK` (status 400 — initial failure but retry succeeded), `500 service down: <slug>, …` (completely failed); `200 OK (no recent data)` when no files exist in the retention window; Run Test and Test All are unaffected and continue to run live probes against all endpoints regardless of region
