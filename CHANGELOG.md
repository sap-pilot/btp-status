...
- **Endpoint `region` filter** — endpoints in `config.json` can now declare an optional `region` field (e.g. `"region": "us10"`); when set, `GET /health/:name` and manual **Run Test** only check that endpoint if the incoming request hostname contains `cfapps.<region>.hana`; this allows a single `config.json` to define all regions while each deployed btp-status instance probes only its local endpoints; the scheduler always checks all endpoints regardless of region; region is extracted from the `x-forwarded-host` (or `Host`) header
...
### Fixed
- **XSUAA login: auth state not updating after successful login** — the popup's callback script now broadcasts the login event via `BroadcastChannel('btpauth')` in addition to `window.opener.postMessage`; in Chrome 94+, cross-origin navigation through the XSUAA authorization page nullifies `window.opener` in the popup, so `postMessage` was silently dropped and the UI never reflected the session cookie the server had already set; the `BroadcastChannel` channel delivers the event to all same-origin tabs regardless of opener state; the `targetOrigin` used for `postMessage` is also fixed to derive from the actual request headers (`x-forwarded-host`/`x-forwarded-proto`) instead of the cached `VCAP_APPLICATION.application_uris[0]`, so it matches the browser origin even when multiple CF routes or custom domains are in use; the `watchPopup → fetchMe()` fallback remains for browsers that do not support `BroadcastChannel`
